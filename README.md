...
- ✅ **智能去重**：自动识别已下载视频，避免重复
- ✅ **无感雷达**：(v5.6.0 优化) 默认关闭，支持在控制台中即时启停的静默雷达监控
...
本次版本重点完成了对微信视频号新版页面的适配，并同步修复了首页、`feed` 页、`profile` 页、`赞和收藏` 页面、评论采集、Hub 搜索与雷达控制等一系列稳定性问题：

- **新版页面适配**:
  - 适配新版首页顶部四标签结构：`直播 / 关注 / 朋友 / 推荐`
  - 修复首页刷新卡在“加载中”、页面空白、下载按钮失效等问题
  - 修复 `feed` 页切换上下一个视频后，下载与评论采集仍指向上一条视频的问题
  - 修复 `profile` 与 `account/like` 页面批量下载按钮位置和当前标签数据同步
- **评论采集增强**:
  - 兼容新版评论侧栏结构
  - 支持自动打开评论区后再开始采集
  - 优化评论滚动加载、进度提示与保存命名
- **Hub / WebSocket 稳定性提升**:
  - 本地页面主动上报当前页面、URL、API 就绪状态和方法能力
  - Hub 自动优先选择真正支持搜索 / 列表 / 详情的设备
  - Dashboard / Search 页面增加设备能力展示，减少“在线但不可用”的误选
- **雷达体验优化**:
  - 雷达默认关闭，更安全
  - 新增 `radar_enabled` 配置项
  - 本地控制台设置页支持即时启停雷达，无需重启
