...
- **底层架构重构**：核心代码拆分（API/Upload），模块化设计，提升稳定性与可维护性。
- **安全性增强**：修复 SSRF 漏洞、JWT 安全问题、并发竞争条件等多项核心安全隐患。
- **WebSocket 统一**：全链路迁移至 `coder/websocket`，彻底解决连接稳定性问题。
- **性能优化**：修复资源泄漏（Goroutine/Timer），优化数据库查询（N+1问题），提升系统响应速度。

### ✨ 主要修复

- 修复了文件上传/下载过程中的潜在数据竞争问题
- 优化了 HTTP 客户端连接池管理
- 修复了部分数据库查询缺失 LIMIT 的问题
- 统一了错误处理和日志记录规范
